...
def main() -> None:
    ...
    try:
        q0 = sim.getJointPosition(joint_handle)
        print(f"Initial joint position q0: {q0:.6f} radians")
    except Exception as e:
    ...
    print("\n[STEP 7] Computing tracking performance...")
    if errors:
        avg_error = sum(errors) / len(errors)
        print(f"Average absolute tracking error: {avg_error:.6f} radians")
        print(f"Number of samples: {len(errors)}")
        
        if avg_error < 0.05:
            print("[PASS] Average tracking error < 0.05 rad. Joint control is working well.")
        else:
            print("[WARN] Average tracking error >= 0.05 rad. Joint may not be in position control mode or has high friction/inertia.")
    else:
        print("[WARN] No error samples collected. Cannot compute tracking performance.")
